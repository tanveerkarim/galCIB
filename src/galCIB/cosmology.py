...
class Cosmology:
    def __init__(self, zs, ks, Mh, colossus_cosmo_name="planck18", use_little_h=False):
        """
    ...
    def _generate_mesh_grid(self, use_mesh_grid=True):
        ...
        self.k_grid, self.Mh_grid, self.z_grid = np.meshgrid(
            self.k, self.Mh, self.z, indexing="ij"
        )
    ...
    def _load_colossus(self):
        ...
        for i in range(len(self.z)):
            tmp_pk = cosmo.matterPowerSpectrum(k_range, self.z[i], model="camb")

            # Convert dn/dlnM to dn/dlog_10M, using ln(10) factor.
            tmp_hmf = mass_function.massFunction(
                x=Mh_range,
                z=self.z[i],
                mdef="200m",
                model="tinker08",
        ...
        tmp_dlnpk_dlnk = cosmo.matterPowerSpectrum(
            k_range,
    ...
    def _cache_quantities(self):
        ...
        # rho_crit
        self.rho_crit = self.cosmo.rho_c(self.z) * (
            u.Msun * cu.littleh**2 / u.kpc**3
        )  # units of Msol * h^2/kpc^3
        self.rho_crit = self.rho_crit.to(
            u.Msun / u.Mpc**3, cu.with_H0(self.H0 * u.km / u.s / u.Mpc)
        ).value  # convert to units of Msol/Mpc^3

        # mean_density of matter at (z=0)
        self.mean_density0 = self.cosmo.rho_c(0) * (
            u.Msun * cu.littleh**2 / u.kpc**3
        )  # units of Msol * h^2/kpc^3
        self.mean_density0 = self.mean_density0.to(
            u.Msun / u.Mpc**3, cu.with_H0(self.H0 * u.km / u.s / u.Mpc)
        ).value  # convert to units of Msol/Mpc^3
    ...
    def _calculate_chi(self):
        ...
        for i in range(len(self.z)):
            chi[i] = self.cosmo.comovingDistance(0, self.z[i])
    ...
    def save_grids(self, filename):
        np.savez(
            filename, pk=self.pk_grid, z=self.z, k=self.k, hmf=self.hmf_grid, Mh=self.Mh
        )
