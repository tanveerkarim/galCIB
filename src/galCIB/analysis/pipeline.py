...
class AnalysisModel:
    ...
    def _kPk_interpolator(self, pk):
        ...
        self.kz_grid = (self.survey.ells[:, np.newaxis] + 0.5) / self.cosmo.chi[
            np.newaxis, :
        ]
        ...
        for zidx in range(len(self.survey.z)):
            pk_interp[:, zidx] = np.interp(
                self.kz_grid[:, zidx], self.cosmo.k, pk[:, zidx]
            )
    ...
    def update_cl(
        ...
        pgg, pII, pgI, pgmu, pmuI = self.pk.compute_pk(
            theta_cen=theta_cen,
            theta_sat=theta_sat,
