...
def sfr_default(BAR_grid, z_ratio):
    ...
    def eta_fn(M, z, theta_eta, **kwargs):
        ...
        sigmaM_b = np.where(
            M_b < Mpeak_b, sigmaM0, np.broadcast_to(sigmaM_z, M_b.shape)
        )  # (..., Nz)
    ...
    def sfr_model(M, z, theta_eta):
        eta_vals = eta_fn(M, z, theta_eta, z_ratio=z_ratio)

        if eta_vals.shape != BAR_grid.shape:
            raise ValueError(
                f"Shape mismatch: eta_vals {eta_vals.shape}, BAR_grid {BAR_grid.shape}"
            )
