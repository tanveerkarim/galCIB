...
def snu_M21_factory(selected_freqs, cosmo, fdata="../data/filtered_snu_planck.fits"):
    ...
    def select_rows_by_frequency(snu, selected_freqs):
        ...
        planck_freqs = np.array(
            [100.0, 143.0, 217.0, 353.0, 545.0, 857.0]
        )  # shape (6,)
        ...
        try:
            ...
            raise ValueError(
                f"Requested frequency {e.args[0]} not in the list of available frequencies."
            )
        return snu[indices]

    snu_subselect = select_rows_by_frequency(snu_eff, selected_freqs)

    interpolator = CubicSpline(redshifts, snu_subselect, axis=1, extrapolate=False)
