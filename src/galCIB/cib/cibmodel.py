...
class CIBModel:
    ...
    def __init__(self, hod_IR, sfr_model, snu_model, subhalo_mf=None, Nm_sub=98):
        self.hod_IR = hod_IR
        ...
        log10_m_min = 5
        self.m_sub_grid = np.logspace(
            log10_m_min, self.log10_Mhc, Nm_sub
        )  # (Nm_sub, NMh)

        self.dlog10m = np.log10(
            self.m_sub_grid[1, :] / self.m_sub_grid[0, :]
        )  # shape: (NMh,)
        self.m_over_Mhc = (self.m_sub_grid / self.Mhc[None, :])[
            ..., None
        ]  # (Nm_sub, NMh,1)
        ...
        self.subhalo_BAR_grid = _compute_BAR_grid(
            cosmo=self.cosmo, Mh=None, m=self.m_sub_grid
        )  # (Nm, NMh)
    ...
    def update(self, theta_sfr, theta_snu, theta_hod_IR):
        ...
        self._dj_central = self._compute_djc(
            self._sfr, self._snu, self._Ncen_IR
        )  # (Nnu, Nm, Nz)
        self._dj_sub = self._compute_djsub(
            self._sfr, self._snu, theta_sfr
        )  # (Nnu, Nm, Nz)
    ...
    def _compute_djsub(self, sfr, snu, theta_sfr):
        ...
        # Calculate SFR(m)
        sfrII = self.sfr_model.evaluate_from_BAR(
            self.subhalo_BAR_grid, self.m_sub_grid, theta_sfr
        )  # (Nmsub, NMh,1)
